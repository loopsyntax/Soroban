...
w=(new StellarSdk.TransactionBuilder(C,{fee:1E5,networkPassphrase:"Test SDF Network ; September 2015"})).addOperation(K.call("play",(new StellarSdk.Address(B)).toScVal(),u)).setTimeout(30).addMemo(StellarSdk.Memo.text("Soroban Snooker")).build(),p.yield(g.prepareTransaction(w),8);case 8:w=p.yieldResult;if(null==(L=b)?0:L.canSign()){w.sign(b);p.jumpTo(9);break}return p.yield(freighterApi.signTransaction(w.toEnvelope().toXDR("base64"),{networkPassphrase:"Test SDF Network ; September 2015"}),10);case 10:M=
p.yieldResult,w=new StellarSdk.Transaction(M,"Test SDF Network ; September 2015");case 9:return p.yield(g.sendTransaction(w),11);case 11:z=p.yieldResult,I=z.hash;case 12:if("PENDING"!==z.status&&"NOT_FOUND"!==z.status){"SUCCESS"===z.status?(E=StellarSdk.xdr.TransactionMeta.fromXDR(z.resultMetaXdr.toXDR().toString("base64"),"base64"),r=E.v3().sorobanMeta().returnValue(),a.score=StellarSdk.scValToNative(r)):(console.error(JSON.stringify(z)),l=!0);p.leaveTryBlock(2);break}return p.yield(new Promise(function(N){return setTimeout(N,
1E3)}),15);case 15:return p.yield(g.getTransaction(I),16);case 16:z=p.yieldResult;p.jumpTo(12);break;case 3:F=p.enterCatchBlock(),console.error(F),l=!0;case 2:h=!1,p.jumpToEnd()}})};a.busy=function(){return h};a.errored=function(){return l};a.networkPassphrase=function(){return"Test SDF Network ; September 2015"};a.testSecret=function(){return"S---ECRET"};a.clearError=function(){l=!1};y.jumpToEnd()})})(window.SnookerContract=window.SnookerContract||{});
...
5);case 5:d=e.yieldResult,d.networkPassphrase!==SnookerContract.networkPassphrase()?a.walletStatus=a.WalletStatus.NotTestnet:a.walletStatus=a.WalletStatus.Connected,e.jumpToEnd()}})};a.resize=function(){if(t){var c=window.innerWidth,d=window.innerHeight;a.scene={x:0,y:0,height:15,width:10,margin:1,isLandscape:c>=d};t.width=a.scene.isLandscape?2048:1152;t.height=a.scene.isLandscape?2048*d/c:1152*d/c;t.style.width=c+"px";t.style.height=d+"px";a.ratio={x:c/t.width,y:d/t.height};a.unitCamera.sx=128;a.unitCamera.sy=
128;a.unitCamera.update();a.size=p({x:t.width,y:t.height});a.unit=Math.max(a.size.x,a.size.y)*(a.scene.isLandscape?.018:.022);a.practiceBtn={x:.5*a.size.x-8.2*a.unit,y:.5*a.size.y-a.unit,width:8*a.unit,height:4*a.unit};a.insertCoinBtn={x:.5*a.size.x+.2*a.unit,y:.5*a.size.y-a.unit,width:8*a.unit,height:4*a.unit};a.playDemoBtn={x:.5*a.size.x-4.05*a.unit,y:.5*a.size.y+9.5*a.unit,width:8.1*a.unit,height:2.7*a.unit};a.closeBtn={x:.5*a.size.x-2.025*a.unit,y:.5*a.size.y+9.5*a.unit,width:4.05*a.unit,height:2.7*
a.unit};a.githubBtn={x:a.size.x-8.1*a.unit,y:0,width:8.1*a.unit,height:5.4*a.unit};a.litemintBtn={x:a.size.x-8*a.unit,y:a.size.y-3.5*a.unit,width:6.9*a.unit,height:2.3*a.unit}}};K=function(c){SnookerContract.busy()||SnookerContract.errored()||a.strikeSolver.status!==a.StrikeStatus.Paused||(a.hudPoint=p(c),a.hudPoint.isDown=!0,c={x:a.hudPoint.x/a.ratio.x,y:a.hudPoint.y/a.ratio.y},D(c,a.githubBtn)?a.githubBtn.isDown=!0:D(c,a.litemintBtn)?a.litemintBtn.isDown=!0:a.stageScreen===a.StageScreen.Score?D(c,
a.closeBtn)&&(a.closeBtn.isDown=!0):a.stageScreen===a.StageScreen.Intro?D(c,a.playDemoBtn)&&(a.playDemoBtn.isDown=!0):SnookerContract.table||(D(c,a.insertCoinBtn)?a.insertCoinBtn.isDown=!0:D(c,a.practiceBtn)&&(a.practiceBtn.isDown=!0)))};w=function(c){if(!SnookerContract.busy()&&!SnookerContract.errored()){var d,e=null==(d=a.hudPoint)?void 0:d.isDown;a.hudPoint=p(c);a.hudPoint.isDown=e}};L=function(){var c,d,e;return $jscomp.asyncExecutePromiseGeneratorProgram(function(k){switch(k.nextAddress){case 1:if(SnookerContract.busy())return k["return"]();
...
!0,"start")):(a.stageScreen===a.StageScreen.Menu?(b.save(),b.translate(0,a.menuAnimTime*a.size.y),f(a.practiceBtn),b.drawImage(a.sprites,0,1024,512,256,a.practiceBtn.x,a.practiceBtn.y,a.practiceBtn.width,a.practiceBtn.height),b.restore(),b.save(),b.translate(0,a.menuAnimTime*a.size.y),f(a.insertCoinBtn),a.walletStatus!==a.WalletStatus.Connected&&(h("Need Freighter wallet","rgb(130,240,255)",a.insertCoinBtn.x+.5*a.insertCoinBtn.width,a.insertCoinBtn.y+4.7*a.unit,.65),h("on Testnet to play","rgb(130,240,255)",
